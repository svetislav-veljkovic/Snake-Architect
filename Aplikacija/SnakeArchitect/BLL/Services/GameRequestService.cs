...
namespace BLL.Services
{
    public class GameRequestService : IGameRequestService
    {
        ...
        public async Task<(bool Success, string Message, int PlayerId, int RoomId, int JoinedUserId, string JoinedUsername)> AcceptGameRequestAsync(int requestId, int userId)
        {
            ...
            var visibleCountAfterJoin = currentPlayers.Count(CanReconnect) + 1;
            if (visibleCountAfterJoin >= room.MinPlayers)
            {
                var pendingRoomRequests = _unitOfWork.GameRequest
                    .Find(gr => !gr.Accepted &&
                                gr.ID != request.ID &&
                                gr.GameRoomId == request.GameRoomId)
                    .ToList();
