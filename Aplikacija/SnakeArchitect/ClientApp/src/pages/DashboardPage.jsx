...
export default function DashboardPage() {
  ...
  const returnableRooms = useMemo(
    () =>
      rooms.filter((room) =>
        room.isStarted &&
        room.isMember &&
        room.myPlayerIsConnected !== false &&
  ...
  useEffect(() => {
    ...
    connection.on("JoinedGameRoom", (roomId) => {
      ...
      setPendingJoinRoomIds((current) => {
        ...
        next.delete(roomId);
        try {
          localStorage.setItem(PENDING_KEY_PREFIX + user.userId, JSON.stringify([...next]));
        } catch {}
        return next;
      });
  ...
  const refreshDashboard = useCallback(async () => {
    ...
    if (pendingJoinRoomIds.size > 0) {
      ...
      if (joinedRoomId !== null) {
        persistPendingJoinIds(stillPending);
        setPendingJoinRoomIds(stillPending);
  ...
  useEffect(() => {
    ...
    api
      .get(`/api/User/${user.userId}/history?limit=5`)
      .then((data) => {
        ...
        <main className="dashboard-main">
          {returnableRooms.length > 0 && (
            <section className="panel resume-panel">
              <div>
                <p className="eyebrow">Partija u toku</p>
                <h2>Vrati se u igru</h2>
              </div>
              <div className="room-list">
                {returnableRooms.map((room) => (
                  ...
                  >
                    ...
                    <span className="room-item-action">Vrati se</span>
                  </button>
                ))}
              </div>
            </section>
          ...
          <section className="panel">
            ...
            <div className="room-list">
              {availableRooms.length === 0 && (
                <p className="muted">Nema dostupnih soba. Sobe se prikazuju kada host potvrdi tablu.</p>
              )}
