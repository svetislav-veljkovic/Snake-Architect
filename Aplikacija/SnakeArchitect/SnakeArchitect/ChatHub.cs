        ...
        public override async Task OnDisconnectedAsync(Exception? exception)
        {
            ...
            if (userId.HasValue)
            {
                ...
                if (newCount == 0)
                {
                    ...
                    var roomIds = await MarkStartedRoomPlayersDisconnectedAsync(userId.Value);
                    foreach (var roomId in roomIds)
                    {
                        await Clients.Group("game:" + roomId)
                            .SendAsync("PlayerConnectionChanged", userId.Value, false);
                    }
                }
            }
            await base.OnDisconnectedAsync(exception);
        }
        private async Task<List<int>> MarkStartedRoomPlayersDisconnectedAsync(int userId)
        {
            var playerRows = _unitOfWork.Player
                .Find(p => p.UserId == userId && p.IsConnected)
                .ToList();
            if (playerRows.Count == 0)
                return new List<int>();
            var roomIds = playerRows.Select(p => p.GameRoomId).Distinct().ToList();
            var startedRoomIds = _unitOfWork.GameRoom
                .Find(r => roomIds.Contains(r.ID) && r.isActive && r.IsStarted)
                .Select(r => r.ID)
                .ToHashSet();
            var affectedPlayers = playerRows
                .Where(p => startedRoomIds.Contains(p.GameRoomId))
                .ToList();
            ...
            return affectedPlayers.Select(p => p.GameRoomId).Distinct().ToList();
        }
